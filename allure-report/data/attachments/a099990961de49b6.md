...
- Location: /Users/gosouza/projetos-p/open-invest-spotlight-dao/tests/e2e/journeys.spec.ts:73:3
...
Call log:
  - waiting for locator('text=0%') to be visible

    at /Users/gosouza/projetos-p/open-invest-spotlight-dao/tests/e2e/journeys.spec.ts:83:16
```
...
```ts
   ...
   22 |     await page.waitForSelector('text=Bem-vindo');
   23 |     await expect(page.getByText('Bem-vindo')).toBeVisible();
   24 |     await page.getByRole('button', { name: 'Complete Step' }).click();
   25 |     
   26 |     // Complete choose journey step
   27 |     await page.waitForSelector('text=Escolha sua Jornada');
   28 |     await expect(page.getByText('Escolha sua Jornada')).toBeVisible();
   29 |     await page.getByRole('button', { name: 'Complete Step' }).click();
   30 |     
   31 |     // Complete how it works step
   32 |     await page.waitForSelector('text=Como Funciona');
   33 |     await expect(page.getByText('Como Funciona')).toBeVisible();
   34 |     await page.getByRole('button', { name: 'Complete Step' }).click();
   35 |     
