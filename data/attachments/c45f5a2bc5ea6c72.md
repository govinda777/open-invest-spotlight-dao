...
- Name: Critical Path Tests >> should load the main page
- Location: /home/runner/work/open-invest-spotlight-dao/open-invest-spotlight-dao/tests/critical.spec.ts:4:3
...
Expected pattern: /Open Invest Spotlight DAO/
Received string:  "open-invest-spotlight-dao"
Call log:
  - expect.toHaveTitle with timeout 10000ms
  - waiting for locator(':root')
    13 × locator resolved to <html lang="en">…</html>
       - unexpected value "open-invest-spotlight-dao"

    at /home/runner/work/open-invest-spotlight-dao/open-invest-spotlight-dao/tests/critical.spec.ts:6:24
```
...
```ts
   ...
   2 |
   3 | test.describe('Critical Path Tests', () => {
   4 |   test('should load the main page', async ({ page }) => {
   5 |     await page.goto('/');
>  6 |     await expect(page).toHaveTitle(/Open Invest Spotlight DAO/);
     |                        ^ Error: Timed out 10000ms waiting for expect(locator).toHaveTitle(expected)
   7 |   });
   8 |
   9 |   test('should display main navigation', async ({ page }) => {
  10 |     await page.goto('/');
  11 |     await expect(page.locator('nav')).toBeVisible();
  12 |   });
  13 |
  14 |   test('should allow user authentication', async ({ page }) => {
  15 |     await page.goto('/login');
  16 |     await page.fill('input[name="email"]', 'test@example.com');
